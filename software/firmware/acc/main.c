...
//unsigned short s1[MAX_MEASURES*2] __attribute__((space(dma))) = {};
//unsigned short s2[MAX_MEASURES*2] __attribute__((space(dma))) = {};
//unsigned short s3[MAX_MEASURES*2] __attribute__((space(dma))) = {};
//unsigned short s4[MAX_MEASURES*2] __attribute__((space(dma))) = {};
//
...
void CalcUniqueID(void)
{
	int i;
	device.uniqueid = 0;
	for(i = 0; i < strlen(timestamp); i++)
		device.uniqueid += timestamp[i];	
}	
...
int main(void)
{
	...
	eth0.ip_id = (WORD)device.uniqueid;
...
//			if (device.packet_timeout_counter++ > device.packets_until_timeout)
//			{
//				asm("nop");
//				asm("nop");
//				asm("nop");
				...
				if (ip_header->protocol == IP_TYPE_UDP)
				{
					...
					if (dport == 7000) // port konfiguracyjne
					{
						...
						if (command == '!') // 'ABCD'
						{
							...
							*((DWORD*)data) = device.uniqueid; data += sizeof(DWORD);
							...
							*((DWORD*)data) = eth0.netmask; data += sizeof(DWORD);
							...
							len += sizeof(DWORD) + sizeof(WORD) + 5*sizeof(DWORD) + 6 + strlen(timestamp) + 1 + strlen(version) + 1;
