...
//	unsigned char q = ENC_ReadControlRegister(EIE);
//	asm("nop");
//	asm("nop");
//	asm("nop");
...
{
	LED2 = 0;
//	WORD length_old = length;

#if 1
	ENC_SetCurrentBank(0x00);

	ENC_WriteRegister_MACRO(EWRPTL, TX_START & 0xFF);
	ENC_WriteRegister_MACRO(EWRPTH, TX_START >> 8);
	
	ENC_WriteRegister_MACRO(ETXNDL, (TX_START + length) & 0xFF);
	ENC_WriteRegister_MACRO(ETXNDH, (TX_START + length) >> 8);
#else
...
#if 1
	// bajt 0x00 - okreselenie sposobu postepowania przy nadawaniu; 0x00 = polegaj na rejestrach konfiguracyjnych 
	while(SPI2STATbits.SPITBF);
	dummy = SPI2BUF;
	SPI2BUF = 0x00; // pierwszy bajt
...
void ENC_SendPacket2B(const unsigned char* buffer1, unsigned int length1, const unsigned char* buffer2, unsigned int length2)
{
	LED2 = 0;
	...
	WORD _length = length1 + length2;
	ENC_WriteRegister_MACRO(ETXNDL, (TX_START + _length) & 0xFF);
	ENC_WriteRegister_MACRO(ETXNDH, (TX_START + _length) >> 8);
